    ...
    for (var j = 0; j < roles.length; j++) {
        for (var k = 0; k < types.length; k++) {
            data.push({
                [KEYS.FORM_ID]: `${uuid}`,
                [KEYS.FORM_STATUS]: `${i}`,
                [KEYS.FORM_NAME]: `${roles[j]} ${types[k]}`,
                ...
                [KEYS.FORM_SUP_NAME]: `${roles[(j + 1) % roles.length]} ${types[(k + 1) % types.length]}`,
                ...
                [KEYS.FORM_SAM_SUBMIT]: `SAM : ${roles[j]} ${types[k]}`,
                [KEYS.FORM_SAM_RECEIVE]: `SAM : ${roles[j]} ${types[k]}`,
                [KEYS.FORM_SAM_SUPER]: `SAM : ${roles[(j + 1) % roles.length]} ${types[(k + 1) % types.length]}`
            });
            uuid++;
        }
    }
...
// requests for users are involved but not in state to approve
// -> is submitter but not manager or recipient
// -> is approver and not in approval state
// -> is recipient and not in recipient state
exports.GetRequestsForSelf = function(role, type) {
    return data.filter(i =>
      (i[KEYS.FORM_SAM_SUBMIT] === `SAM : ${role} ${type}` &&
        !(i[KEYS.FORM_SUP_NAME] === `${role} ${type}` ||
        i[KEYS.FORM_NAME] === `${role} ${type}`)) ||
      (+ i[KEYS.FORM_STATUS] !== 1 && i[KEYS.FORM_SUP_NAME] === `${role} ${type}`) ||
      (+ i[KEYS.FORM_STATUS] !== 2 && i[KEYS.FORM_NAME] === `${role} ${type}`)
   );
}

// requests that require approval
// -> is approver and in manager approval state
// -> is recipient and in recipient approval state
// -> is security role and in security approval state, but not recipent
exports.GetRequestsNeedingApproval = function(role, type) {
    return data.filter(i =>
      (+ i[KEYS.FORM_STATUS] === 1 && i[KEYS.FORM_SUP_NAME] === `${role} ${type}`) ||
      (+ i[KEYS.FORM_STATUS] === 2 && i[KEYS.FORM_NAME] === `${role} ${type}`) ||
      (+ i[KEYS.FORM_STATUS] === 3 && role === roles[2] && !(i[KEYS.FORM_NAME] === `${role} ${type}`))
    );
}
